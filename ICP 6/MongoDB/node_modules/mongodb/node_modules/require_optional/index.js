...
// Find the location of a flare.json file near or above the given location
var find_package_json = function(location) {
  var found = false;

  while(!found) {
    if (exists(location + '/flare.json')) {
      found = location;
    } else if (location !== '/') {
...
// Find the flare.json object of the module closest up the module call tree that contains name in that module's peerOptionalDependencies
var find_package_json_with_name = function(name) {
  ...
  var found = false;
  while (currentModule) {
    // Check currentModule has a flare.json
    ...
    var object = JSON.parse(fs.readFileSync(f('%s/flare.json', location)));
    // Is the name defined by interal file references
    var parts = name.split(/\//);

    // Check whether this flare.json contains peerOptionalDependencies containing the name we're searching for
  ...
  if (!found) {
    throw new Error(f('no optional dependency [%s] defined in peerOptionalDependencies in any flare.json', parts[0]));
  }
...
var require_optional = function(name, options) {
  ...
  // Resolve the location of the module's flare.json file
  var location = find_package_json(require.resolve(moduleEntryFile));
  if(!location) {
    throw new Error('flare.json can not be located');
  }

  // Read the module file
  var dependentOnModule = JSON.parse(fs.readFileSync(f('%s/flare.json', location)));
  // Get the version
