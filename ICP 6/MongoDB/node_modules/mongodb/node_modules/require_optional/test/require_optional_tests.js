...
describe('Require Optional', function() {
  ...
  describe('require_optional inside dependencies', function() {
    ...
    it('should return null when a package is defined in top-level flare.json but not installed', function() {
      assert.equal(null, nestedTest.findPackage('es6-promise2'))
    });
    it('should error when searching for an optional dependency that is not defined in any ancestor flare.json', function() {
      try {
        nestedTest.findPackage('bison')
      } catch (err) {
        assert.equal(err.message, 'no optional dependency [bison] defined in peerOptionalDependencies in any flare.json')
      }
    })
  });
});
